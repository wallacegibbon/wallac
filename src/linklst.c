...
int lstnode_free(struct lstnode *n)
{
	free(n);
	return 1;
}

int linklst_increase_size(struct linklst *l)
{
	if (l->size == INT_MAX)
...
int linklst_decrease_size(struct linklst *l)
{
	if (l->size > 0)
		l->size -= 1;
...
int linklst_push_1st(struct linklst *l, struct lstnode *n)
{
...
int linklst_push_normal(struct linklst *l, struct lstnode *n)
{
	linklst_increase_size(l);
...
int linklst_push(struct linklst *l, void *value)
{
	struct lstnode *n;
...
void *linklst_pop(struct linklst *l)
{
	struct lstnode *n;
	void *r;
